...
function editThisNumber (numberIdVar){
    var email = $("#email"+numberIdVar).text();
    var ip = $("#ip"+numberIdVar).text();
    var number = $("#number"+numberIdVar).text();
    numberId = numberIdVar;
    numberGlobal = number;
    ...
    $("#addEditFormName").text('Edit record form');
...
function deleteThisNumber (recordId){
    swalInit.fire({
        title: 'Are you sure?',
        ...
        if (confirmed.isConfirmed) {
            $.ajax({
                ...
                success:function(data) {
                    var removingRow = $("#number"+recordId).parent();
                    table.row(removingRow).remove().draw(false);
                    sweetAlert("Successfully deleted a record!", "Success", 'success');
                },
...
function addEdit(){
    ...
    if(add){
        $.ajax({
            ...
            success:function(data) {
                table.row.add([counter,number,ip,email,`
                    <div class="d-inline-flex">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <a href="#" class="dropdown-item text-primary" onclick="editThisNumber('`+data[0]['id']+`')">
                                    <i class="ph-pencil-simple me-2"></i>
                                    Edit
                                </a>
                                <a href="#" class="dropdown-item text-danger" onclick="deleteThisNumber('`+data[0]['id']+`')">
                                    <i class="ph-x me-2"></i>
                                    Remove
                                </a>
                                <a href="#" class="dropdown-item text-success" onclick="showOneNumberScores('`+data[0]['id']+`')">
                    ...
                    <input type="hidden" id="_id`+number+`" value="`+data[0]['id']+`"/>
                `]).draw(false)
                    .nodes()
                    .to$()
                    .find('td')
                    .each(function(index, element) {
                        switch(index) {
                            case 0:
                                $(this).attr('id', 'iteration'+data[0]['id']  );
                                break;
                            case 1:
                                $(this).attr('id', 'number'+data[0]['id']  );
                                break;
                            case 2:
                                $(this).attr('id', 'ip'+data[0]['id']  );
                                break;
                            case 3:
                                $(this).attr('id', 'email'+data[0]['id']  );
                                break;
                            case 4:
                ...
                sweetAlert("Successfully added new record!", "Success", 'success');
                $("#newNumber").val("");
                $("#newEmail").val("");
                $("#newIp").val("");
            },
            error: function (error){
                sweetAlert('Some unexpected error happened, please make sure record have at least one value!', "Error", 'error');
                console.log(error);
            }
        });
        add = true;
        ...
        $.ajax({
            ...
            data:{
                ...
                email: email,
                ip: ip,
                recordId: parseInt(numberId)
            },
            success:function(data) {
                // console.log(data);
                // return;
                // console.log(data[0]['numberId']);
                // console.log(numberLocal);
                // return;
                someId = numberLocal;
                var editingRow = $('#iteration'+data[0]['id']).text();
                newData = [editingRow, number, ip, email, `
                    <div class="d-inline-flex">
                        <div class="dropdown">
                            <a href="#" class="text-body" data-bs-toggle="dropdown">
                                <i class="ph-list"></i>
                ...
                table.row((editingRow-1)).data( newData )
                    .draw(false)
                    .nodes()
                    .to$()
                    .find('td')
                ...
                sweetAlert("Successfully edited record!", "Success", 'success');
                $("#newNumber").val("");
                $("#newEmail").val("");
                $("#newIp").val("");
                numberLocal = "";
    ...
    $("#addEditFormName").text('Add new record form');
...
function checkBeforeSubmit(){
    if((table.data().count() === 0)){
        sweetAlert('You need to add at least one record to score!', "Error", 'error');
        return
    }

    swalInit.fire({
        title: 'Are you sure?',
        text: 'Are you sure you want to score this numbers?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonText: 'Yes, do the scoring!'
    }).then((confirmed) => {
        if (confirmed.isConfirmed) {
            $("#scoreForm").submit();
            swalInit.fire({
                html:
                    ...
                    '</div>' +
                    '<h2>Scoring the data, please wait</h2>',
